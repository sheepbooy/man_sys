...
from management.resources import authorization_resource
...
def authorization(request):
    ...
    if search_fields and search_values:
        ...
        query_set = models.Authorization.objects.all()  # 确保是您的模型名
    ...
    # 准备模型字段信息传递到模板
    field_info = [(field.name, field.verbose_name) for field in models.Authorization._meta.fields if field.name != 'id']
...
def authorization_edit(request, _id):
    """编辑授权书总表"""
    row_object = models.Authorization.objects.filter(id=_id).first()
    if request.method == 'GET':
...
def authorization_delete(request, _id):
    """授权表删除"""
    models.Authorization.objects.filter(id=_id).delete()
    last_emp_page = request.session.get('last_emp_page', '/authorization/')
    return redirect(last_emp_page)


@permission_required('management.change_authorization', '/warning/')
def authorization_export(request):
    """导出"""
    selected_fields = request.GET.get('fields', None)
    _resource = authorization_resource()
...
@permission_required('management.change_authorization', '/warning/')
def authorization_import(request):
    """导入"""
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = authorization_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            authorization_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/authorization/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/authorization/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/authorization/import/'})
