...
from management.resources import butting_resource
...
def butting(request):
    ...
    if search_fields and search_values:
        ...
        query_set = models.CustomerEngagement.objects.all()  # 确保是您的模型名
    ...
    # 准备模型字段信息传递到模板
    field_info = [(field.name, field.verbose_name) for field in models.CustomerEngagement._meta.fields if
                  field.name != 'id']
...
def butting_edit(request, _id):
    """编辑研发部客户对接表"""
    row_object = models.CustomerEngagement.objects.filter(id=_id).first()
    if request.method == 'GET':
...
def butting_delete(request, _id):
    """删除研发部客户对接信息"""
    models.CustomerEngagement.objects.filter(id=_id).delete()
...
@permission_required('management.change_customerengagement', '/warning/')
def butting_export(request):
    """导出"""
    selected_fields = request.GET.get('fields', None)
    product_resource = butting_resource()
...
def butting_import(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = butting_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            butting_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/develop/butting/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/develop/butting/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/develop/butting/import/'})
