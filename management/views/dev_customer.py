...
from management.resources import dev_custom_resource
...
def dev_custom_edit(request, _id):
    """编辑研发部客户档案"""
    row_object = models.CustomerProfile.objects.filter(id=_id).first()
    if request.method == 'GET':
...
def dev_custom_delete(request, _id):
    """删除研发部客户信息"""
    models.CustomerProfile.objects.filter(id=_id).delete()
    last_emp_page = request.session.get('last_emp_page', '/develop/customer/')
    return redirect(last_emp_page)


@permission_required('management.change_customerprofile', '/warning/')
def dev_custom_export(request):
    """导出"""
    selected_fields = request.GET.get('fields', None)
    _resource = dev_custom_resource()
...
def dev_custom_import(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = dev_custom_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            dev_custom_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/develop/customer/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/develop/customer/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/develop/customer/import/'})
