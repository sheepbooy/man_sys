...
from management.resources import ForeignCustomerResource
...
def foreign_customer_export(request):
    ...
    _resource = ForeignCustomerResource()
...
def foreign_customer_import(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = ForeignCustomerResource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            ForeignCustomerResource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
