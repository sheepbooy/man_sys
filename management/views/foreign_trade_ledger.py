...
from management.resources import foreign_trade_ledger_resource
...
def foreign_trade_ledger(request):
    ...
    # 准备模型字段信息传递到模板
    field_info = [(field.name, field.verbose_name) for field in models.ForeignTradeLedger._meta.fields if
                  field.name != 'id']
...
def foreign_trade_ledger_export(request):
    """导出"""
    selected_fields = request.GET.get('fields', None)
    _resource = foreign_trade_ledger_resource()
...
@permission_required('management.change_products', '/warning/')
def foreign_trade_ledger_import(request):
    """导入"""
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = foreign_trade_ledger_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            foreign_trade_ledger_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/foreign/ledger/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/foreign/ledger/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/foreign/ledger/import/'})
