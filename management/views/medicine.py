...
from management.resources import medicine_resource
...
def medicine(request):
    ...
    if search_fields and search_values:
        ...
        query_set = models.Medicine.objects.all()  # 确保是您的模型名
    ...
    # 准备模型字段信息传递到模板
    field_info = [(field.name, field.verbose_name) for field in models.Medicine._meta.fields if field.name != 'id']
...
@permission_required('management.change_medicine', '/warning/')
def medicine_export(request):
    """导出"""
    selected_fields = request.GET.get('fields', None)
    _resource = medicine_resource()
...
def medicine_import(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = medicine_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            medicine_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/medicine/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/medicine/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/medicine/import/'})
