...
from management.resources import question_resource
...
def question_import(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = question_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            question_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/question/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/question/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/question/import/'})
