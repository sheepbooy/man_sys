...
from management.resources import turnover_resource
...
def turnover(request):
    ...
    if search_fields and search_values:
        ...
        query_set = models.CustomerFlow.objects.all()  # 确保是您的模型名
    ...
    # 准备模型字段信息传递到模板
    field_info = [(field.name, field.verbose_name) for field in models.CustomerFlow._meta.fields if field.name != 'id']
...
def turnover_add(request):
    ...
    if request.method == 'GET':
        ...
        back_url = request.session.get('last_emp_page', '/develop/turnover/')
        # 确保将back_url传递给模板
    ...
    if form.is_valid():
        form.save()
        last_emp_page = request.session.get('last_emp_page', '/develop/turnover/')
...
def turnover_edit(request, _id):
    """编辑研发部客户流水表"""
    row_object = models.CustomerFlow.objects.filter(id=_id).first()
    if request.method == 'GET':
...
def turnover_delete(request, _id):
    """删除研发部客户流水信息"""
    models.CustomerFlow.objects.filter(id=_id).delete()
...
@permission_required('management.change_customerflow', '/warning/')
def turnover_export(request):
    """研发部客户流水表导出"""
    selected_fields = request.GET.get('fields', None)
    _resource = turnover_resource()
...
def turnover_import(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        result = turnover_resource().import_data(imported_data, dry_run=True)  # 先试运行

        if not result.has_errors():
            # 实际导入数据
            turnover_resource().import_data(imported_data, dry_run=False)
            # 导入成功，构建JSON响应
            response_data = {
                'message': '数据成功导入！',
                'redirect': request.session.get('last_emp_page', '/develop/turnover/')
            }
    ...
    back_url = request.session.get('last_emp_page', '/develop/turnover/')
    # 确保将back_url传递给模板
    return render(request, 'import.html', {'back_url': back_url, 'redirect_url': '/develop/turnover/import/'})
